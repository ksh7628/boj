 ...
 * 2-1. 내리막 경사로의 경우, 좌측 높이가 1 더 큰 경우에만 L 길이만큼 놓을 수 있는지 검사하고 가능하면 해당 구간을 isBuild 배열에 true로 저장
 * 2-2. 오르막 경사로인 경우, 우측 높이가 1 더 큰 경우에만 L 길이만큼 놓을 수 있는지 검사한다. 이 때, 이미 경사로가 건설된 구간이면 false 반환
 * 2-3. 두 경우 모두 경사로를 건설할 수 있다면 h에 현재 높이를 갱신하고 L길이 만큼 스킵(i +- L-1)
 * 2-4. 도중에 높이가 2 이상 차이나는 구간은 건설할 수 없으므로 false를 반환, 경사로를 건설하는 과정에서 높이가 같지 않거나 배열 인덱스를 벗어나는 경우에도 false를 반환
